...
class BaseDataset:
    ...
    def format(self, fewshot: int = 0, include_cot: bool = True):
        def _format(
            example: Dict,
            ...
            if use_answer:
                if include_cot:
                    ...
                    query += ('' if query[-1] in {'\n', ' '} else ' ') + self.direct_template(a)
            return query

        # demo
        demo = [{
            'question': self.examplars[i]['question'],
            'case': _format(self.examplars[i], use_answer=True, input_template_func=self.demo_input_template, include_cot=include_cot),
...
class WikiMultiHopQA(BaseDataset):
    ...
    output_template = lambda self, cot, ans: f'{cot} Final answer: "{ans}".'
    direct_template = lambda self, ans: f'Final answer: "{ans}".'
    final_answer_str = "Final answer:"
    prompt_answer_str = "Final answer:"

    def __init__(self, data_path: str, filename: str = "dev.json", start_index: int = 0): 
        logger.info(f"Loading WikiMultiHopQA from {data_path}/{filename}")
        dataset = []

        start = False if start_index != 0 else True
        with open(os.path.join(data_path, filename), 'r') as fin:
            js = json.load(fin)
            for idx, example in tqdm(enumerate(js)):
                if not start:
...
class Musique(BaseDataset):
    ...
    def __init__(self, data_path: str, filename: str = "final_dev.json", start_index: int = 0):
        logger.info(f"Loading Musique from {data_path}/{filename}")
        dataset = []
        start = False if start_index != 0 else True
        with open(os.path.join(data_path, filename), "r") as fin:
            js = json.load(fin)
...
class HotpotQA(BaseDataset):
    ...
    def __init__(self, data_path: str, filename: str = "hotpotqa-dev.json", start_index: int = 0):
        logger.info(f"Loading HotpotQA from {data_path}/{filename}")
        dataset = []
        start = False if start_index != 0 else True
        with open(os.path.join(data_path, filename), "r") as fin:
            js = json.load(fin)
            for idx, example in tqdm(enumerate(js)):
                if not start:
                    if (idx + 1) == start_index:
                        start = True
                    continue
                qid = example["_id"] if "_id" in example else idx
                question = example["question"]
                answer = example['answer']
    ...
    direct_template = lambda self, ans: f'So the answer is {ans}.'
    final_answer_str = "the answer is"
    prompt_answer_str = " So the answer is "
